...
confusionMatrixPlot2 <- function(cnn_matrix2) {
  # Convert the confusion matrix to a data frame for ggplot
  cm_df2 <- as.data.frame(as.table(cnn_matrix2$table))
  colnames(cm_df2) <- c('Prediction', 'Reference', 'Freq')

  # Plot using ggplot
  ggplot(data = cm_df2, aes(x = Reference, y = Prediction, fill = Freq)) +
    geom_tile(color = "white") +
    scale_fill_gradient(low = "white", high = "steelblue") +
    geom_text(aes(label = sprintf("%0.0f", Freq)), vjust = 1) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(fill = "Count")+
    scale_y_discrete(limits = rev(levels(factor(cm_df2$Prediction)))) 
}
...
# Start with a simple fit call
rnn_history <- rnn_model %>% fit(x_train_set, y_train_set)
...
```
After training, the model is evaluated on the test dataset, where its performance is quantified using loss and accuracy metrics. The RNN model achieves a test loss of 0.2164 and an accuracy of 92.21%, indicating its efficiency in classifying fashion items.
...
plot(rnn_history$metrics$accuracy, type = "l", col = "red", ylim = c(0, 1), xlab = "Epochs", ylab = "Accuracy", main = "Training and Validation Accuracy")
lines(rnn_history$metrics$val_accuracy, col = "blue")
legend("bottomright", legend = c("Training", "Validation"), col = c("red", "blue"), lty = 1)

# Plotting training and validation loss
plot(rnn_history$metrics$loss, type = "l", col = "red", xlab = "Epochs", ylab = "Loss", main = "Training and Validation Loss")
lines(rnn_history$metrics$val_loss, col = "blue")
legend("bottomright", legend = c("Training", "Validation"), col = c("red", "blue"), lty = 1)
```
...
confusionMatrixPlot2 <- function(rnn_conf_matrix) {
  # Convert the confusion matrix to a data frame for ggplot
  rnn_conf_df <- as.data.frame(as.table(rnn_conf_matrix$table))
  colnames(cm_df2) <- c('Prediction', 'Reference', 'Freq')

  # Plot using ggplot
  ggplot(data = rnn_conf_df, aes(x = Reference, y = Prediction, fill = Freq)) +
    geom_tile(color = "white") +
    scale_fill_gradient(low = "white", high = "steelblue") +
    geom_text(aes(label = sprintf("%0.0f", Freq)), vjust = 1) +
    theme_minimal() +
...
RNN Model: The RNN, employing LSTM units, also performed admirably, achieving an accuracy of 92.21% with a test loss of 0.2164. Its ability to process data sequentially made it adept at handling image classification, although slightly less effective than the CNN in this context.
...
knitr::purl("Fashion2.Rmd", output = "Fashion2.R")
